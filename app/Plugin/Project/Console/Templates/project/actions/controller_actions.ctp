        ...
        foreach (array('belongsTo', 'hasAndBelongsToMany') as $assoc):
            foreach ($modelObj->{$assoc} as $associationName => $relation):
                if (!empty($associationName)):
                    ...
                    echo "\t\t\${$otherPluralName} = \$this->{$currentModelName}->{$otherModelName}->find('list', array('fields'=>array(\$this->{$currentModelName}->{$otherModelName}->displayField)));\n";
...
<?php endif; ?>
    ...
    function admin_import(){
		...
		if (isset($this->request->data['<?php echo $currentModelName; ?>']['file']['tmp_name']) &&
			is_uploaded_file($this->request->data['<?php echo $currentModelName; ?>']['file']['tmp_name'])) {
			$destination = $path . '/'. $this->request->data['<?php echo $currentModelName; ?>']['file']['name'];
			move_uploaded_file($this->request->data['<?php echo $currentModelName; ?>']['file']['tmp_name'], $destination);
			$filename = getcwd().'/'.$this->request->data['<?php echo $currentModelName; ?>t']['file']['name'];
     ...
     function admin_getlist(){
		...
		$items = $this-><?php echo $currentModelName ?>->find('all');
		$items = Hash::combine($items, '{n}.<?php echo $currentModelName ?>.id', '{n}');
		$newitems = array();
		foreach($items as $i => $item){
			$item['<?php echo $currentModelName ?>']['text'] = $item['<?php echo $currentModelName ?>'][$this-><?php echo $currentModelName ?>->displayField];
			$newitems[] = $item['<?php echo $currentModelName ?>'];
		}
    ...
    function admin_export(){
		$this->autoRender = false;
		$data = $this-><?php echo $currentModelName; ?>->find('all');
    ...
    function mobileadd() {
		$this->autoRender = false;
		$this-><?php echo $currentModelName ?>->create();
		if ($this-><?php echo $currentModelName ?>->save($_POST)) {
			$check = array(
			'logged' => false,
			'message' => 'Saved!',
    ...
    function mobiledelete($id = null) {
     ...
     function <?php echo $admin ?>savehabtmfld(){
